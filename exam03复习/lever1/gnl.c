...
char	*gnl_strjoin(char const *s1, char const *s2)
{
	...
	if (!str)               /* malloc 失败直接返回 NULL */
		return (NULL);
	i = -1;                 /* 使用 size_t，这里设为 -1 然后在循环中 ++i，使得 i 从 0 开始 */
	while (++i < len1)      /* 复制 s1 到新字符串 */
		str[i] = s1[i];
	i = -1;                 /* 重置 i 用来复制 s2 */
	while (++i < len2)      /* 将 s2 复制到新字符串的后半部分 */
		str[len1 + i] = s2[i];
	str[len1 + len2] = '\0';/* 添加终止符 */
	return (str);           /* 返回新拼接的字符串 */
}
...
static char	*join_free(char *s1, char *s2)
{
	...
	char	*tmp;          /* 暂存原 s1（原代码中 tmp = s1） */

	tmp = s1;               /* 暂存旧指针以便传入 gnl_strjoin */
	joined = gnl_strjoin(tmp, s2); /* 拼接 tmp 与 s2（内部会分配新内存） */
	free(tmp);              /* 释放旧的 s1 内存（注意 tmp 可能为 NULL，但 free(NULL) 是安全的） */
...
char	*read_save(int fd, char *backup)
{
	...
	{
		count = read(fd, buf, BUFFER_SIZE); /* 从 fd 读取最多 BUFFER_SIZE 字节 */
		...
		backup = join_free(backup, buf);  /* 将 buf 拼接到 backup（并释放旧 backup） */
		if (!backup)                      /* 如果拼接后返回 NULL（malloc 失败） */
